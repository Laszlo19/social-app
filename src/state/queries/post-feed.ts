      ...
      const {api, cursor} = pageParam
        ...
        : {
            api: createApi({
              ...
              feedTuners,
              client,
              // Fork: current user's DID, so the likes branch can tell self
              // (getActorLikes) from others (public like records).
...
function createApi({
  ...
  if (feedDesc === 'following') {
     ...
     */
    if (actor && actor === currentDid) {
      return new LikesFeedAPI({
        client,
        feedParams: {actor: actor as AtIdentifierString},
      })
    }
    return new PublicActorLikesFeedAPI({client, actor})
