...
/**
 * Creates a single authenticated lex client for the given account without
 * switching the active session. Used by the fork's multi-account actions
 * ("Like as @x" / "Repost as @x"). Throws if the session can't be resumed.
 */
export async function createEphemeralClient(
