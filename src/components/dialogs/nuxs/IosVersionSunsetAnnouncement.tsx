...
export function IosVersionSunsetAnnouncement() {
   ...
   *
   * When manually triggered from Developer Options, `activePreview.variant`
   * forces a specific variant so both can be previewed on any platform (on
   * Android IOS_MAJOR_VERSION is always 0, which would otherwise only ever show
   * the iOS 15 copy).
   */
  const previewVariant = nuxDialogs.activePreview?.variant
  const isOnIos16 = previewVariant
    ? previewVariant === 'ios16'
